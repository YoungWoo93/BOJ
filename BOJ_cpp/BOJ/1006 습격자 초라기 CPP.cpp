...
bool isMergeable(node a, node b, int count)
{
	if (!a.isMerge && !b.isMerge && a.enemyCount + b.enemyCount <= count)
		return true;
...
int getTeamCount(vector<node>* arr, int index, int memberCount, int teamCount)
{
	if (index == arr[0].size())
		return teamCount == 0 ? 1 : teamCount;
	...
	node& outprv = arr[1][prv];
	node& inprv = arr[0][prv];
	node& outnode = arr[1][index];
	node& innode = arr[0][index];



	if (isMergeable(outprv, outnode, memberCount)) // 외곽 잇기
	{
		Marge(outprv, outnode);
		ret = getTeamCount(arr, index + 1, memberCount, teamCount - 1);
		split(outprv, outnode);
	}
	if (isMergeable(inprv, innode, memberCount)) // 내곽 잇기
	{
		Marge(inprv, innode);
		ret = min(ret, getTeamCount(arr, index + 1, memberCount, teamCount - 1));
		split(inprv, innode);
	}
	if (isMergeable(innode, outnode, memberCount)) // 상하 잇기
	{
		Marge(innode, outnode);
		ret = min(ret, getTeamCount(arr, index + 1, memberCount, teamCount - 1));
		split(innode, outnode);
	}
	if (isMergeable(outprv, outnode, memberCount) && isMergeable(inprv, innode, memberCount)) // 내외곽 잇기
	{
		Marge(inprv, innode);
		Marge(outprv, outnode);
		ret = min(ret, getTeamCount(arr, index + 1, memberCount, teamCount - 2));
		split(outprv, outnode);
		split(inprv, innode);
	}
	// 안잇기
	ret = min(ret, getTeamCount(arr, index + 1, memberCount, teamCount));
...
int main()
{
	...
	for (int i = 0; i < caseCount; i++)
	{
		...
		cout << "[" << getTeamCount(part, 0, memberCount, partCount * 2) << "]" << endl;
	}
